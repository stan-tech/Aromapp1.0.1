...
namespace Aromapp
{
    public partial class AdminOptions : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void AdminOptions_Load(object sender, EventArgs e)
        {
            ...
            CustCheck.Checked = Properties.Settings.Default.CustLocked;
            SuppCheck.Checked = Properties.Settings.Default.SuppLocked;
            PrdCheck.Checked = Properties.Settings.Default.PrdLocked;
        ...
        private void CaisseCheck_CheckedChanged(object sender, EventArgs e)
        {
            if (loading)
            {
                return;
            ...
            string page = ctrl.Name;


            void Confirm(object send, EventArgs eve)
            {
                Confirm confirm = new Confirm();
               ...
               if( confirm.ShowDialog() == DialogResult.Cancel)
                {
                    Guna2CustomCheckBox box = ctrl as Guna2CustomCheckBox;
                    box.CheckedChanged -= CaisseCheck_CheckedChanged;
            ...
            void PasswoCorrectDelete(object s, EventArgs ev)
            {
                switch (page)
                {
                    case "CmptCheck":
                        Properties.Settings.Default.CmptLocked = CmptCheck.Checked;
                        break;
                    case "SalesCheck":
                        Properties.Settings.Default.SalesLocked = SalesCheck.Checked;

                        break;
                    case "AchatCheck":
                        Properties.Settings.Default.AchatLocked = AchatCheck.Checked;

                        break;
                    case "PrdCheck":
                        Properties.Settings.Default.PrdLocked = PrdCheck.Checked;

                        break;
                    case "SuppCheck":
                        Properties.Settings.Default.SuppLocked = SuppCheck.Checked;

                        break;
                    case "CustCheck":
                        Properties.Settings.Default.CustLocked = CustCheck.Checked;

                        break;
                    case "CaisseCheck":
                        Properties.Settings.Default.CaisseLocked = CaisseCheck.Checked;
